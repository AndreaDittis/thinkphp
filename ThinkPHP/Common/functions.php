...
function U($url = '', $vars = '', $suffix = true, $redirect = false, $domain = true) {
		...
		foreach ( $routes as $rule => $r_route ) {
			...
			if (url_compare ( $_url, $_route )) {
				foreach ( $_route as $key => $val ) {
					if ($key == C ( 'VAR_MODULE' ) || $key == C ( 'VAR_ACTION' ))
						continue;
					if (preg_match ( "/~/", $val )) {
	...
	if ($flag) { // PATHINFO模式或者兼容URL模式
		if (empty ( $vars ) && C ( 'DEFAULT_MODULE' ) == $var [C ( 'VAR_MODULE' )] && C ( 'DEFAULT_ACTION' ) == $var [C ( 'VAR_ACTION' )]) {
		...
		if($urlMode == URL_COMPAT ){
			// 兼容模式判断
			$urlbase = _PHP_FILE_.'?'.C('VAR_PATHINFO').'=';
		}elseif($urlMode == URL_REWRITE ) {
			//当前项目地址
			$urlbase    =   dirname(_PHP_FILE_);
			if($urlbase == '/' || $urlbase == '\\')
				$urlbase    =   '';
		}
		if (isset ( $route )) {
			$url = $urlbase . '/' . rtrim ( $url, $depr );
		} else {
			$url = $urlbase . '/' . implode ( $depr, array_reverse ( $var ) );
		}
		if (! empty ( $vars )) { // 添加参数
